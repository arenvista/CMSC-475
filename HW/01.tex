...
\[
	\begin{gathered}
		...
		\text{Consider: } \alpha'(x) := \frac{1}{1+e^{-x}} \implies \forall ~x \in \mathbb{R}, ~a'(x) \in (0,1) \approx [0,1]
	\end{gathered}
\]
...
\[
	\begin{gathered}
		...
		\end{array}
		\right . \\\\
		\tiny{(\text{Note: } \theta := w_n)} \implies y_i = \alpha(\theta^T x_i) = \alpha(\theta \cdot x_i) \\
		\therefore ~P(y_{i}|x_{i},\theta) = \alpha(\theta^T x_i)
	\end{gathered}
\]
...
\[
	\begin{gathered}
		\text{Recall (Sum Sq. Errors): } J(\theta) = \frac{1}{N}\sum_{i=1}^{N} (y_i-\theta^Tx_i)^2 \\
		\theta^Tx_i \text{ is a dot product yielding the } \hat{y_i}
	\end{gathered}
\]
...
% 			\frac{\mathscr{d}J(\theta)}{d \theta_1} \\[1em]
% 			\frac{\mathscr{d}J(\theta)}{d \theta_1} \\[1em]
% 			\vdots                                  \\[1em]
% 			\frac{\mathscr{d}J(\theta)}{d \theta_M} \\[1em]
% 		\end{bmatrix} \\
% 		\text{Recall (Sum Sq. Errors): } J(\theta) = \frac{1}{N}\sum_{i=1}^{N} (y_i-\theta^Tx_i)^2 \\
% 		\theta^Tx_i \text{ is a dot product yielding the } \hat{y_i} \\
% 	\end{gathered}
...
(A) Express the gradient equation in simple English.    \\
(B) Assuming that one has performed a forward pass to obtain a prediction, how many computations (how many multiplications and how many additions/subtractions) would one need to do to obtain the gradient for each weight?
...
(1) ( ) 001  \\
(1) ( ) 0021  \\
$\frac{\partial J^{(1)}(\theta)}{\partial\theta_{3}}$  \\
...
def divide_dataset (data_x, data_y, train_percentage): 
    # your code for dividing the dataset
...
\begin{lstlisting}[language=Python]
def gradient_descent(w, b, alpha, data_x, data_y): 
    # do forward pass 
    # compute gradients 
    # update w and b 
    return new_w, new_b 
...
\begin{table}[h]
	...
	\begin{tabular}{|c|c|c|c|c|}
		\hline
		                 & $N_{1}=10000$ & $N_{2}=20000$ & $N_{3}=$ & $N_{4}=$ \\ \hline
		$\alpha_{1}=0.1$ &               &               &          &          \\ \hline
		$\alpha_{2}=$    &               &               &          &          \\ \hline
		$\alpha_{3}=$    &               &               &          &          \\ \hline
		$\alpha_{4}=$    &               &               &          &          \\ \hline
...
$$x(m,n)=\left[\begin{matrix}1&10\\ 2&20\\ 3&30\end{matrix}\right] \quad h(m,n)=\left[\begin{matrix}1&0&1\\ -1&0&-1\end{matrix}\right]$$
